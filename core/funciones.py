...
def get_datos_email_html(datos, userDestino, subject, template_name, archivo=[]):
    from pryictiair.settings import EMAIL_HOST_USER
...
def custom_get_timezone(request, user=None):
    import pytz
    from pryictiair.settings import TIME_ZONE
...
def addData(request, data):
    ...
    from pryictiair.settings import BASE_DIR, URL_GENERAL
    ...
    data['email_notificacion'] = confi.email_notificacion.url if confi.email_notificacion else ""
